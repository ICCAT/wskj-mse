...
```{r, fig.cap = "Selectivity of GOM_larvae in last historical year."}
if(!is.null(report_list[[1]]$ivul)) {
ivul_ff_age <- sapply(report_list, function(x) x$ivul[nyears, , 4])
matplot(age, ivul_ff_age, type = "l", col = scenario$col2, xlab = "Age", ylim = c(0, 1), ylab = "Selectivity of GOM_larvae")
abline(h = 0, col = "grey")
if(!is.null(scenario$names)) legend("topleft", scenario$names, col = scenario$col2, lty = scenario$lty, lwd = scenario$lwd)
}
```
...
```{r, fig.cap = "Observed (black) and predicted (red) index values for GOM_larvae."}
Ipred <- sapply(report_list, function(x) x$Ipred[, 4])
matplot(Year, Ipred, type = "l", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, ylim = c(0, 1.1 * max(c(Ipred, RCMdata@Index[, 4]), na.rm = TRUE)), xlab = "Year", ylab = "GOM_larvae")
...
```{r, fig.cap = "Observed (black) and predicted (red) index values for GOM_larvae. Error bars indicate 95% confidence intervals for observed values."}
if(length(RCMdata@I_sd) && any(RCMdata@I_sd > 0, na.rm = TRUE)) {
  II <- RCMdata@Index[, 4]
  ind <- seq(min(which(!is.na(II))), max(which(!is.na(II))), 1)
  err <- exp(log(II) + outer(RCMdata@I_sd[, 4], c(-1.96, 1.96)))
  matplot(Year[ind], Ipred[ind, , drop = FALSE], type = "l", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, ylim = c(0, 1.1 * max(c(Ipred[ind, ], II[ind], err[ind, ]), na.rm = TRUE)), xlab = "Year", ylab = "GOM_larvae")
  points(Year[ind], II[ind], lwd = 3, pch = 16)
  arrows(Year[ind], y0 = err[ind, 1], y1 = err[ind, 2], length = 0, lwd = 1.5)
  abline(h = 0, col = "grey")
  if(!is.null(scenario$names)) legend("topleft", scenario$names, col = scenario$col, lty = scenario$lty, lwd = scenario$lwd)
...
```{r, fig.cap = "Observed (black) and predicted (red) mean ages from GOM_larvae."}
...
```{r, fig.cap = "Observed (black) and predicted (red) mean lengths from GOM_larvae."}
...
```{r, fig.cap = "Observed (black) and predicted (red) age composition from GOM_larvae."}
if(length(RCMdata@IAA) && any(RCMdata@IAA[, , 4] > 0, na.rm = TRUE)) {
...
```{r, fig.cap = "Observed (black) and predicted (red) length composition from GOM_larvae."}
if(length(RCMdata@IAL) && any(RCMdata@IAL[, , 4] > 0, na.rm = TRUE)) {
...
matplot(Year, Ipred, type = "l", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, ylim = c(0, 1.1 * max(c(Ipred, RCMdata@Index[, 5]), na.rm = TRUE)), xlab = "Year", ylab = "HL_RR")
...
if(!is.null(report_list[[1]]$ivul)) {
ivul_ff_age <- sapply(report_list, function(x) x$ivul[nyears, , 6])
...
Ipred <- sapply(report_list, function(x) x$Ipred[, 6])
matplot(Year, Ipred, type = "l", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, ylim = c(0, 1.1 * max(c(Ipred, RCMdata@Index[, 6]), na.rm = TRUE)), xlab = "Year", ylab = "BB_West_Hist")
lines(Year, RCMdata@Index[, 6], col = "black", typ = "o")
abline(h = 0, col = "grey")
if(!is.null(scenario$names)) legend("topleft", scenario$names, col = scenario$col, lty = scenario$lty, lwd = scenario$lwd)
```
...
if(length(RCMdata@I_sd) && any(RCMdata@I_sd > 0, na.rm = TRUE)) {
  II <- RCMdata@Index[, 6]
  ind <- seq(min(which(!is.na(II))), max(which(!is.na(II))), 1)
  err <- exp(log(II) + outer(RCMdata@I_sd[, 6], c(-1.96, 1.96)))
...
if(length(RCMdata@IAA) && any(RCMdata@IAA[, , 6] > 0, na.rm = TRUE)) {
MApred <- sapply(report_list, function(x) x$IAApred[, , 6] %*% age/rowSums(x$IAApred[, , 6]))
MAobs <- (RCMdata@IAA[, , 6] %*% age)/rowSums(RCMdata@IAA[, , 6], na.rm = TRUE)
ylim <- c(0.9, 1.1) * range(c(MApred, MAobs), na.rm = TRUE)
matplot(Year, MApred, type = "l", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = "Year", ylab = "Mean age", ylim = ylim)
if(any(RCMdata@IAA[, , 6] > 0, na.rm = TRUE)) {
  lines(Year, MAobs, col = "black", typ = "o")
}
if(!is.null(scenario$names)) legend("topleft", scenario$names, col = scenario$col, lty = scenario$lty, lwd = scenario$lwd)
}
...
if(length(RCMdata@IAL) && any(RCMdata@IAL[, , 6] > 0, na.rm = TRUE)) {
MLpred <- sapply(report_list, function(x) x$IALpred[, , 6] %*% RCMdata@Misc$lbinmid/rowSums(x$IAApred[, , 6]))
MLobs <- (RCMdata@IAL[, , 6] %*% RCMdata@Misc$lbinmid)/rowSums(RCMdata@IAL[, , 6], na.rm = TRUE)
ylim <- c(0.9, 1.1) * range(c(MLpred, MLobs), na.rm = TRUE)
matplot(Year, MLpred, type = "l", col = scenario$col, lty = scenario$lty, lwd = scenario$lwd, xlab = "Year", ylab = "Mean length", ylim = ylim)
if(any(RCMdata@IAL[, , 6] > 0, na.rm = TRUE)) {
  lines(Year, MLobs, col = "black", typ = "o")
}
if(!is.null(scenario$names)) legend("topleft", scenario$names, col = scenario$col, lty = scenario$lty, lwd = scenario$lwd)
}
...
if(length(RCMdata@IAA) && any(RCMdata@IAA[, , 6] > 0, na.rm = TRUE)) {
pred_IAA <- sapply(report_list, function(x) x$IAA[, , 6], simplify = "array") %>% aperm(perm = c(3, 1, 2))
plot_composition_RCM(Year, pred_IAA, RCMdata@IAA[, , 6], ages = age, dat_col = scenario$col)
}
```
...
if(length(RCMdata@IAL) && any(RCMdata@IAL[, , 6] > 0, na.rm = TRUE)) {
pred_IAL <- sapply(report_list, function(x) x$IAL[, , 6], simplify = "array") %>% aperm(perm = c(3, 1, 2))
plot_composition_RCM(Year, pred_IAL, RCMdata@IAL[, , 6], CAL_bins = length_bin, dat_col = scenario$col)
}
```
...
plot_timeseries(Year, RCMdata@Index[, 4], report$Ipred[, 4], label = "GOM_larvae")
```

```{r, fig.cap = "Observed (black) and predicted (red) index from survey 5."}
plot_timeseries(Year, RCMdata@Index[, 5], report$Ipred[, 5], label = "HL_RR")
```

```{r, fig.cap = "Observed (black) and predicted (red) index from survey 6."}
plot_timeseries(Year, RCMdata@Index[, 6], report$Ipred[, 6], label = "BB_West_Hist")
```
...
plot_residuals(Year[istart:istop], log(RCMdata@Index[, 4][istart:istop]/report$Ipred[, 4][istart:istop]), label = "GOM_larvae Residuals")
```
...
plot_residuals(Year[istart:istop], log(RCMdata@Index[, 5][istart:istop]/report$Ipred[, 5][istart:istop]), label = "HL_RR Residuals")
```

```{r, fig.cap = "Index residuals (in log space) for index from survey 6."}
istart <- which(!is.na(RCMdata@Index[, 6]))[1]
istop <- which(!is.na(RCMdata@Index[, 6])) %>% max()
plot_residuals(Year[istart:istop], log(RCMdata@Index[, 6][istart:istop]/report$Ipred[, 6][istart:istop]), label = "BB_West_Hist Residuals")
```
